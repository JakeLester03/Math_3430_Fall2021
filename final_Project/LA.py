...
def scalar_vector_mult(vector: list[float], scalar: float) -> list:   
    '''
    ...
    '''
    result: list[float] = [0 for element in vector]
...
def absolute_value(scalar: complex) -> complex:
    '''
    ...
    to get the absolute value. In the case of real numbers, thre is no imaginary part

    Args:
        A scalar stored in a computer as either a complex or real number.
...
def boolean_norm(vector: list[float], p: float=2, boolean: bool = False) -> float:
    '''
...
def inner_product(vector_a:list[complex], vector_b:list[complex]) -> complex:
    ...
    result:list[complex] = [0 for elements in vector_a]
